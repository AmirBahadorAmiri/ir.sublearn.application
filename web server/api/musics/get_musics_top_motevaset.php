...
$array_result["musics_motevaset"] = array();

$sqlmanager->sendQuery("SELECT * FROM `app_musics_motevaset` where is_enabled=1 limit 7");

if ( $sqlmanager->getResult()->num_rows > 0 ) {
    $array_result["result_code"] = 1;
    while ( $row = $sqlmanager->getResult()->fetch_assoc() ) {
        array_push($array_result["musics_motevaset"], $row);
    }
}
